...
const App: React.FC = () => {
  ...
  const renderBooleanIcon = (value: boolean) => (
    <span className={`inline-block w-4 h-4 rounded-full ${value ? "bg-green-500" : "bg-red-500"}`} />
  )
        ...
        <div className="border border-white/20 rounded p-4">
          {activeTab === "barcode" ? (
            ...
            <>
              ...
              {ingredientsLoading ? (
                <div className="text-center text-white/80 animate-pulse">ANALYZING INGREDIENTS...</div>
              ) : ingredientsError ? (
                <div>
